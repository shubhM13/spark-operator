...
// signal to this reconciler's named MutatingWebhookConfiguration, or nil when no
// CA event channel is registered. A CA source event carries no object, so the
...
func (r *Reconciler) updateMutatingWebhookConfiguration(ctx context.Context, key types.NamespacedName) error {
	...
	if err := r.client.Get(ctx, key, webhook); err != nil {
		...
		return fmt.Errorf("failed to get mutating webhook configuration %v: %w", key, err)
	}
	...
	if err := r.client.Patch(ctx, webhook, client.StrategicMergeFrom(base, client.MergeFromWithOptimisticLock{})); err != nil {
		return fmt.Errorf("failed to patch mutating webhook configuration %v: %w", key, err)
	}

...
func TestUpdateMutatingWebhookConfigurationUsesCABundleSource(t *testing.T) {
	...
	if err := reconciler.updateMutatingWebhookConfiguration(t.Context(), key); err != nil {
...
func TestUpdateMutatingWebhookConfigurationPropagatesCABundleSourceError(t *testing.T) {
	webhook := &admissionregistrationv1.MutatingWebhookConfiguration{}
	webhook.Name = "webhook"
	webhook.Webhooks = []admissionregistrationv1.MutatingWebhook{{Name: "first.example.com"}}
	baseClient := fake.NewClientBuilder().WithScheme(scheme.WebhookScheme).WithObjects(webhook).Build()
	wantErr := errors.New("source unavailable")
	reconciler := NewReconciler(baseClient, fakeCABundleSource{err: wantErr}, webhook.Name)

	err := reconciler.updateMutatingWebhookConfiguration(t.Context(), client.ObjectKey{Name: webhook.Name})
...
func TestUpdateMutatingWebhookConfigurationPatchesDriftAndConverges(t *testing.T) {
	webhook := &admissionregistrationv1.MutatingWebhookConfiguration{}
	webhook.Name = "webhook"
...
func TestReconcilePreservesPatchError(t *testing.T) {
	wantErr := errors.New("patch mutating webhook configuration")
	webhook := &admissionregistrationv1.MutatingWebhookConfiguration{}
	webhook.Name = "webhook"
	webhook.Webhooks = []admissionregistrationv1.MutatingWebhook{{Name: "first.example.com"}}
	baseClient := fake.NewClientBuilder().WithScheme(scheme.WebhookScheme).WithObjects(webhook).Build()
...
func TestUpdateMutatingWebhookConfigurationPreservesPatchError(t *testing.T) {
	wantErr := errors.New("patch mutating webhook configuration")
	webhook := &admissionregistrationv1.MutatingWebhookConfiguration{}
	webhook.Name = "webhook"
	webhook.Webhooks = []admissionregistrationv1.MutatingWebhook{{Name: "first.example.com"}}
...
func TestUpdateMutatingWebhookConfigurationPreservesGetError(t *testing.T) {
	wantErr := errors.New("get mutating webhook configuration")
	...
	err := reconciler.updateMutatingWebhookConfiguration(t.Context(), client.ObjectKey{Name: "webhook"})
...
func TestUpdateMutatingWebhookConfigurationIgnoresNotFound(t *testing.T) {
	baseClient := fake.NewClientBuilder().WithScheme(scheme.WebhookScheme).Build()
	reconciler := NewReconciler(baseClient, certificate.NewProvider(baseClient, "webhook", "default", false), "webhook")

	if err := reconciler.updateMutatingWebhookConfiguration(t.Context(), client.ObjectKey{Name: "webhook"}); err != nil {
		t.Fatalf("expected missing configuration to be terminal, got %v", err)
	}
}

func TestUpdateMutatingWebhookConfigurationPreservesCACertError(t *testing.T) {
	webhook := &admissionregistrationv1.MutatingWebhookConfiguration{}
	webhook.Name = "webhook"
	baseClient := fake.NewClientBuilder().WithScheme(scheme.WebhookScheme).WithObjects(webhook).Build()
...
func TestUpdateMutatingWebhookConfigurationPreservesConflictError(t *testing.T) {
	webhook := &admissionregistrationv1.MutatingWebhookConfiguration{}
	webhook.Name = "webhook"
	webhook.ResourceVersion = "1"
	webhook.Webhooks = []admissionregistrationv1.MutatingWebhook{{Name: "first.example.com"}}
	baseClient := fake.NewClientBuilder().WithScheme(scheme.WebhookScheme).WithObjects(webhook).Build()
	wantErr := apierrors.NewConflict(schema.GroupResource{Group: "admissionregistration.k8s.io", Resource: "mutatingwebhookconfigurations"}, webhook.Name, errors.New("concurrent update"))
	failingClient := interceptor.NewClient(baseClient, interceptor.Funcs{
		Patch: func(context.Context, client.WithWatch, client.Object, client.Patch, ...client.PatchOption) error {
			return wantErr
		},
	...
	err := reconciler.updateMutatingWebhookConfiguration(t.Context(), types.NamespacedName{Name: webhook.Name})
